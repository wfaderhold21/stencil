...
int main(int argc, char ** argv) {
    ...
    a = (float **) shmem_malloc(sizeof(float *) * M);
    b = (float **) shmem_malloc(sizeof(float *) * M);
    c_a = shmemx_malloc_with_hints(sizeof(float) * M * M, SHMEM_HINT_GPU);
    c_b = shmemx_malloc_with_hints(sizeof(float) * M * M, SHMEM_HINT_GPU);
